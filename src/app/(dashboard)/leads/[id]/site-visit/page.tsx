  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-[var(--text-heading)] dark:text-white">Site Visit</h1>
      </div>
      ...
      {!loading && !error && isScheduled && (
        <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-card)] p-6 shadow-sm ">
          ...
          <dl className="mt-4 space-y-3">
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-[var(--text-secondary)]">Date &amp; Time</dt>
              <dd className="mt-1 text-sm text-[var(--text-heading)] dark:text-white">
                {new Date(visit.scheduledAt).toLocaleString('en-IN')}
              </dd>
            </div>
            <div>
              <dt className="text-xs font-medium uppercase tracking-wide text-[var(--text-secondary)]">Address</dt>
              <dd className="mt-1 text-sm text-[var(--text-heading)] dark:text-white">{address}</dd>
            </div>
            {visit.designerId && (
              <div>
                <dt className="text-xs font-medium uppercase tracking-wide text-[var(--text-secondary)]">Designer</dt>
                <dd className="mt-1 text-sm text-[var(--text-heading)] dark:text-white">{visit.designerId}</dd>
              </div>
            )}
            {visit.notes && (
              <div>
                <dt className="text-xs font-medium uppercase tracking-wide text-[var(--text-secondary)]">Notes</dt>
                <dd className="mt-1 text-sm text-[var(--text-heading)] dark:text-white">{visit.notes}</dd>
              </div>
            )}
          </dl>
          {completeError && <p className="mt-3 text-sm text-red-600">{completeError}</p>}
          <button onClick={handleMarkComplete} disabled={completing}
            className="mt-5 rounded-lg bg-[var(--accent-base)] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#6C4FF5] disabled:opacity-50">
