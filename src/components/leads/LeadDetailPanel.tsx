...
const ACTIVITY_STYLE: Record<LeadActivity['type'], { bg: string; color: string }> = {
  call:         { bg: 'var(--accent-soft)', color: 'var(--accent-base)' },
  whatsapp:     { bg: 'var(--success-soft)', color: 'var(--success)' },
  note:         { bg: 'var(--accent-soft)', color: '#9333EA' },
  site_visit:   { bg: 'var(--warning-soft)', color: 'var(--warning)' },
  meeting:      { bg: 'var(--accent-soft)', color: 'var(--accent-base)' },
  stage_change: { bg: '#F1F5F9', color: '#64748B' },
  follow_up:    { bg: 'var(--warning-soft)', color: 'var(--warning)' },
};
...
export function LeadDetailPanel({ lead, onClose }: Props) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <aside
        className={`fixed right-0 top-0 bottom-0 z-50 w-[400px] max-w-[90vw] flex flex-col bg-[var(--surface-card)] shadow-2xl transition-transform duration-200 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      ...
      >
        {!lead ? null : (
          <>
            ...
            {/* ── Quick actions ─────────────────────────────────────── */}
            <div className="flex gap-2 px-5 py-3 flex-shrink-0" style={{ borderBottom: '1px solid var(--surface-muted)' }}>
              <a
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5" style={{ scrollbarWidth: 'none' }}>
              ...
              {/* Quick note input */}
              {showNoteBox && (
                <div className="rounded-xl p-3 space-y-2" style={{ background: '#F9F8FF', border: '1px solid var(--accent-soft)' }}>
                  <textarea
                    value={noteText}
                    ...
                    style={{ background: 'var(--surface-card)', border: '1px solid #DDD6FE', color: '#171923' }}
                    autoFocus
                  />
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-tertiary)' }}>Contact</p>
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-primary)' }}>
                    <Phone className="h-3.5 w-3.5 flex-shrink-0" style={{ color: 'var(--text-tertiary)' }} />
                    <a href={`tel:${lead.contactPhone}`} className="hover:underline">{lead.contactPhone}</a>
                  </div>
                  {lead.contactEmail && (
                    <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-primary)' }}>
                      <MessageCircle className="h-3.5 w-3.5 flex-shrink-0" style={{ color: 'var(--text-tertiary)' }} />
              ...
              {(lead.propertyType || lead.budgetBand || (lead.projectValuePaise ?? 0) > 0) && (
                <div className="space-y-2">
                  <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-tertiary)' }}>Project</p>
                  <div className="space-y-1.5">
                    {lead.propertyType && (
                      <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-primary)' }}>
                    ...
                    {lead.budgetBand && (
                      <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-primary)' }}>
                        <IndianRupee className="h-3.5 w-3.5 flex-shrink-0" style={{ color: 'var(--text-tertiary)' }} />
              ...
              {lead.followUpDate && (
                <div className="space-y-2">
                  <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-tertiary)' }}>Follow-up</p>
                  <div
              ...
              {lead.notes && (
                <div className="space-y-2">
                  <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-tertiary)' }}>Notes</p>
                  <p className="text-sm leading-relaxed" style={{ color: 'var(--text-primary)' }}>{lead.notes}</p>
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                {activitiesLoading ? (
                  ...
                  <p className="text-xs text-center py-4" style={{ color: 'var(--text-tertiary)' }}>No activities recorded yet</p>
                ) : (
                  <div className="space-y-3">
                    {activities.map((activity) => {
                      const Icon = ACTIVITY_ICONS[activity.type] ?? Clock;
                      const style = ACTIVITY_STYLE[activity.type] ?? { bg: 'var(--surface-muted)', color: 'var(--text-primary)' };
                        ...
                        <div key={activity.id} className="flex gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {isCompleted && (
                                <CheckCircle2 className="h-3 w-3 flex-shrink-0" style={{ color: 'var(--success)' }} />
                              )}
                              {activity.status === 'overdue' && (
                                <AlertCircle className="h-3 w-3 flex-shrink-0" style={{ color: 'var(--danger)' }} />
                              )}
                            </div>
